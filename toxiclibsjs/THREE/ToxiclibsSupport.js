...
var is = require('../internals/is');
    ...
    ToxiclibsSupport.createMeshGeometry = function createMeshGeometry(obj, geometry){
        ...
        for (let vertex of vertices) {
          ...
          modVertices.push(vertex.x)
          modVertices.push(vertex.y)
          modVertices.push(vertex.z)
        }
      console.log('mod vertices:', modVertices)
        let bufferVertices = new Float32Array(modVertices)
      ...
      console.log('geometry: ', geometry)
        for( i= 0; i<len; i++ ){
            v = vertices[i];
            geometry.attributes.position.set(i, new THREE.Vector3(v.x, v.y, v.z));
            idIndexMap[v.id] = i;
        }
