...
function lerpColor(textObj, time) {
  if (
    Math.round(textObj.material.color.r * 100) / 100 ==
      Math.round(currentGoalColor.r * 100) / 100 ||
    time == 20
  ) {
    currentGoalColor = new Color(randomColor({ format: "rgb" }));
  }
  textObj.material.color.set(
    currentColor.lerpColors(textObj.material.color, currentGoalColor, 0.03)
  );
  time *= 0.0005;
