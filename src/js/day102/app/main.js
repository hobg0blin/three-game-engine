...
let effect, resolution, light, ambientLight, materials, videoTex
let repeatX = 2
let repeatY = 4
...
/*				'chrome': new THREE.MeshLambertMaterial( { color: 0xffffff, envMap: reflectionCube } ),
        ...
        'flat': new THREE.MeshLambertMaterial( { flatShading: true } ),
                ...
                'webcam': new THREE.MeshPhongMaterial({  map: videoTex, specular: 0x111111, shininess: 1})
/*				'toon1': toonMaterial1,
				'toon2': toonMaterial2,
		...
		function setupGui() {
			...
			h.add( effectController, 'resolution', 0, 100, 0.5 );
			h.add( effectController, 'isolation', 10, 300, 1 );
			h.add( effectController, 'repeatX', 0, 300, 0.25 );
			h.add( effectController, 'repeatY', 0, 300, 0.25 );
    ...
    animate() {
			...
			// marching cubes
        if (effectController.repeatX !== repeatX) {
              repeatX = effectController.repeatX
//              videoTex.repeat.set(repeatX, repeatY)
                materials['webcam'] = new THREE.MeshPhongMaterial({  map: videoTex, specular: 0x111111, shininess: 1})

         }
        if (effectController.repeatY !== repeatY) {
              repeatX = effectController.repeatY
 //             videoTex.repeat.set(repeatX, repeatY)
                materials['webcam'] = new THREE.MeshPhongMaterial({  map: videoTex, specular: 0x111111, shininess: 1})
         }
