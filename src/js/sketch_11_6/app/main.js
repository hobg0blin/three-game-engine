...
// clipping variables
  const Vertices = [
    ...
    GlobalClippingPlanes = cylindricalPlanes( 10, 5 ),
...
//        this.scene.background = backgroundImg
        // CONTROLS
      this.controls = createControls(this.camera, this.renderer, {autorotate: true})
     ...
     // this.composer.addPass( new RenderPass( this.scene, this.camera ) );
      //this.effect1 = new AfterimagePass(0.97)
        ...
        color.setHex( Math.random() * 0xffffff );
				...
				const geometry = new THREE.TorusKnotGeometry( Math.random(), Math.random(), Math.round(Math.random()*24), Math.round(Math.random()*20), Math.round(Math.random()*5), Math.round(Math.random()*5) );
    ...
    animate() {
     ...
     // object.position.y = 1;
        ...
        for (let child of object.children) {
          child.rotation.z += time*Math.random()*0.001
        }
