...
//        this.composer.addPass(this.effect1)
      let geo = new THREE.SphereGeometry(1, 32)
      let mat = new THREE.MeshPhongMaterial({color: Math.random() * 255, wireframe: true })
      let xPos = -12;
      let lineMat = new THREE.LineBasicMaterial({color: 'green'});
      for (let i = 0; i < 8; i++) {
        let mesh = new THREE.Mesh(geo, mat);
        if (i % 2 == 0) {
          xPos += 4;
          mesh.position.set(xPos, 10, 0);
          let lineGeo = new THREE.BufferGeometry();
          let lineMesh = new THREE.Line(lineGeo.setFromPoints([new THREE.Vector3(xPos, 0, 0), new THREE.Vector3(xPos, 10, 0)]), lineMat);
          lineMesh.baseX = xPos;
          lineMesh.forward = true;
          lines.push(lineMesh);
